...
class PDF extends FPDF {
    ...
    function factura($rst, $cns, $emisor) {
        ...
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 19);
        $this->Cell(88, 5, "  No.   " . $rst[fac_numero], 'LR');
        $this->Ln();
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 24);
        ...
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 29);
        $this->Cell(88, 5, $rst[fac_autorizacion], 'LR');
        $this->Ln();
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 34);
        ...
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 39);
        $this->Cell(88, 5, "AUTORIZACION : " . $rst[fac_fec_hora_aut], 'LR');
        $this->Ln();
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 44);
        ...
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 59);
        $this->Code39($x + 115, $y + 59, $rst[fac_clave_acceso]);
        ...
        $this->Ln($x + 4, $y + 1);
        $this->SetFont('helvetica', 'B', 8);
        $this->Cell(141, 5, "Razon Social / Nombres y Apellidos : " . utf8_decode($rst[fac_nombre]), 'LT', 0, 'L');
        $this->SetFont('helvetica', 'B', 8);
        $this->Cell(55, 5, "Identificacion : " . $rst[fac_identificacion], 'RT', 'R');
        $this->Ln();
        $this->SetFont('helvetica', 'B', 8);
        $this->Cell(141, 5, "Fecha de Emision : " . $rst[fac_fecha_emision], 'LB', 0, 'L');
        $this->SetFont('helvetica', 'B', 8);
        $this->Cell(55, 5, "Guia Remision : " . $rst[fac_num_guia_remision], 'RB', 0, 'L');
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        //////////////////////////////////////////////// CUERPO ////////////////////////////////////////////////////////                        
        $this->Ln();
        $this->Ln();
        ...
        while ($rst1 = pg_fetch_array($cns)) {
            ...
            $this->Cell(22, 4, $rst1[dfc_codigo], 1, 0, 'C');
            $this->Cell(21, 4, $rst1[dfc_cod_aux], 1, 0, 'C');
            $this->Cell(13, 4, $rst1[dfc_cantidad], 1, 0, 'C');
            $this->Cell(80, 4, $rst1[dfc_lote] . " " . substr(utf8_decode($rst1[dfc_descripcion]), 0, 40), 1, 0, 'L');
            $this->Cell(20, 4, $rst1[dfc_precio_unit], 1, 0, 'R');
            $this->Cell(20, 4, $rst1[dfc_porcentaje_descuento], 1, 0, 'R');
            $this->Cell(20, 4, $rst1[dfc_precio_total], 1, 0, 'R');
            $this->Ln();
        }
        //___________________________________________________________________________
        $this->Cell(136, 5, "", '', 0, 'L');
        ...
        $this->Cell(20, 5, $rst[fac_subtotal12], 'LRTB', 0, 'R');
        $this->Ln();
        $this->SetFont('helvetica', 'B', 10);
        ...
        $this->Cell(20, 5, $rst[fac_subtotal0], 'LRTB', 0, 'R');
        $this->Ln();
        $this->SetFont('helvetica', '', 8);
        $this->Cell(85, 5, "Vendedor: " . $rst[vnd_nombre], 'LR', 0, 'L');
        $this->Cell(51, 5, "", '', 0, 'L');
        $this->SetFont('helvetica', '', 8);
        $this->Cell(40, 5, "SUBTOTAL No objeto de IVA", 'LRTB', 0, 'L');
        $this->Cell(20, 5, $rst[fac_subtotal_no_iva], 'LRTB', 0, 'R');
        $this->Ln();
        $this->SetFont('helvetica', '', 8);
        $this->Cell(85, 5, "Direccion : " . $rst[fac_direccion], 'LR', 0, 'L');
        $this->Cell(51, 5, "", '', 0, 'L');
        $this->SetFont('helvetica', '', 8);
        ...
        $this->Ln();
        $this->SetFont('helvetica', '', 8);
        $this->Cell(85, 5, "Telefono : " . $rst[fac_telefono], 'LR', 0, 'L');
        $this->Cell(51, 5, "", '', 0, 'L');
        $this->SetFont('helvetica', '', 8);
        $this->Cell(40, 5, "SUBTOTAL Exento de IVA", 'LRTB', 0, 'L');
        $this->Cell(20, 5, $rst[fac_subtotal_ex_iva], 'LRTB', 0, 'R');
        $this->Ln();
        $this->SetFont('helvetica', '', 8);
        $this->Cell(85, 5, "Email :  " . strtolower($rst[fac_email]), 'LR', 0, 'L');
        $this->Cell(51, 5, "", '', 0, 'L');
        $this->SetFont('helvetica', '', 8);
        $this->Cell(40, 5, "DESCUENTO", 'LRTB', 0, 'L');
        $this->Cell(20, 5, $rst[fac_total_descuento], 'LRTB', 0, 'R');
        $this->Ln();
        ...
        $this->Cell(20, 5, $rst[fac_total_ice], 'LRTB', 0, 'R');
        $this->Ln();
        $this->Cell(136, 5, "", '', 0, 'L');
        $this->SetFont('helvetica', '', 8);
        $this->Cell(40, 5, "IVA 12%", 'LRTB', 0, 'L');
        $this->Cell(20, 5, $rst[fac_total_iva], 'LRTB', 0, 'R');
        $this->Ln();
        $this->Cell(136, 5, "", '', 0, 'L');
        $this->SetFont('helvetica', '', 8);
        $this->Cell(40, 5, "IRBPNR", 'LRTB', 0, 'L');
        $this->Cell(20, 5, $rst[fac_total_irbpnr], 'LRTB', 0, 'R');
        $this->Ln();
        $this->Cell(136, 5, "", '', 0, 'L');
        $this->SetFont('helvetica', '', 8);
        $this->Cell(40, 5, "PROPINA", 'LRTB', 0, 'L');
        $this->Cell(20, 5, $rst[fac_total_propina], 'LRTB', 0, 'R');
        ...
        $this->Cell(20, 5, $rst[fac_total_valor], 'LRTB', 0, 'R');
        $this->Ln();

        $this->SetXY(7.2, ($this->GetY() - 25));
        $this->MultiCell(85, 5, "Observaciones :  " . strtoupper(utf8_decode($rst[fac_observaciones])), 'LRB', 1);
    }
...
$Obj = new Mail();
$PDF = new PDF();
$rst_am = pg_fetch_array($Set->lista_configuraciones('5'));
$amb = $rst_am[con_valor];
$id = $_REQUEST[id];
...
$emisor = pg_fetch_array($Set->lista_emisor($em));
$cns = $Set->lista_detalle_factura($id);
$PDF->AddPage();
$PDF->factura($rst, $cns, $emisor, $id);
...
if ($resp == 0) {
    if (!$Set->update_estado_correo('ENVIADO', $id)) {
        ...
        echo "<script> window.location = '../Scripts/Lista_factura_completo.php'</script>";
    }
}
