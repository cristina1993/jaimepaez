...
if (isset($_GET[search])) {
    $desde = $_GET[desde];
    $hasta = $_GET[hasta];
    $cns_data = $Rep->lista_reporte_cheques($desde, $hasta);
} else {
    $desde = date('Y-m-d');
    $hasta = date('Y-m-d');
}
...
<html> 
    <meta charset=utf-8 />
    <title>Planilla de Cheques</title>
    ...
    <body>
        <table style="display:none" border="1" id="tbl2">
            ...
            <tr><td colspan="33" align="center">PLANILLA DE CHEQUES</td></tr>
            <tr>
        ...
        <table  id="tbl" style="width:100%">
            <caption  class="tbl_head" id="cont_head" >
                ...
                <center class="cont_title" >PLANILLA DE CHEQUES</center>
            ...
            <tbody>
                ...
                while ($rst = pg_fetch_array($cns_data)) {
                    ...
                    if ($saldo != 0) {
                        $rst_cliente = pg_fetch_array($Rep->lista_clientes($rst[cli_id]));
                        $f = explode('-', $rst[chq_fecha]);
                        $m = round($f[1]);
                        $fec = $f[2] . '-' . $meses[$m] . '-' . $f[0];
                        $d = date_format(date_create($rst[chq_fecha]), 'N');
                        $s = date_format(date_create($rst[chq_fecha]), 'W');
                        if ($d == 6 || $d == 7) {
                            $s = $s + 1;
                        }
                        $dia = $dias[$d];
                        ...
                        echo "<tr class='row' lang='$s' >
                            ...
                            <td>$rst[chq_concepto]</td>
                            <td>$rst[chq_nombre]</td>
                            <td>$rst[chq_banco]</td>
                            <td align='right'>$rst[chq_numero]</td>
                            <td align='right'>" . number_format($rst[chq_monto], 2) . "</td>";
                        echo "</tr>";
                        $fml = $s;
                        $tm = $rst[chq_monto];
                        $t_mont+= $tm;
