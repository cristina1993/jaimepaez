...
if (isset($_GET[search])) {
    ...
    if (!empty($txt)) {
        $texto = "where  tps_siglas like '%$txt%' or tps_nombre like '%$txt%' or tps_observaciones like '%$txt%'";
    }
    $cns = $Clase_tipoproducto->lista_buscardor_tipoproducto($texto);
} else {
//    $texto = '';
//    $cns = $Clase_tipoproducto->lista_tipoproducto();
}
?>
    ...
    <head>
        <script>
            ...
            function del(id, op)
            {
                ...
                if (r == true) {
                    $.post("actions_tipoproducto.php", {act: 48, id: id, op: op}, function (dt) {
                        if (dt == 0)
                        {
                            window.history.go(0);
                        } else {
